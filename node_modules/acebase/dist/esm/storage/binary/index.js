...
import { NodeChangeTracker, NodeChange } from '../../node-changes.js';
...
import { BinaryBPlusTree, BPlusTreeBuilder, BinaryWriter } from '../../btree/index.js';
...
export class AceBaseStorage extends Storage {
    ...
    async writeData(fileIndex, buffer, offset = 0, length = -1) {
        ...
        const { bytesWritten } = await pfs.write(this.file, buffer, offset, length, fileIndex).catch(err => {
    ...
    async readData(fileIndex, buffer, offset = 0, length = -1) {
        ...
        try {
            const { bytesRead } = await pfs.read(this.file, buffer, offset, length, fileIndex);
            this.stats.reads++;
            ...
            if (options.keyFilter) {
                ...
                for (const { key, value } of results) {
                    const childInfo = isArray
                        ? new BinaryNodeInfo({ path: `${this.address.path}[${key}]`, index: key })
                        : new BinaryNodeInfo({ path: `${this.address.path}/${key}`, key: key });
                    const res = getValueFromBinary(childInfo, value.recordPointer, 0);
                    if (!res.skip) {
                        let result = callback(childInfo, i++);
                ...
                while (leaf) {
                    const children = leaf.entries.reduce((nodes, entry) => {
                        const child = isArray
                            ? new BinaryNodeInfo({ path: `${this.address.path}[${entry.key}]`, index: entry.key })
                            : new BinaryNodeInfo({ path: `${this.address.path}/${entry.key}`, key: entry.key });
