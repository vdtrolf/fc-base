...
export class BPlusTreeNode {
    ...
    gtChild: BPlusTreeNode | BPlusTreeLeaf = null;

    constructor(public tree: BPlusTree, public parent: BPlusTreeNode) {
    }
    ...
    insertKey(newKey: NodeEntryKeyType, fromLeaf: BPlusTreeLeaf, newLeaf: BPlusTreeLeaf) {
    ...
    async toBinary(keepFreeSpace: boolean, writer: BinaryWriter) {
        // EBNF layout:
        ...
        const pointers = [] as BinaryPointer[]; // pointers refer to an offset in the binary data where nodes/leafs can be found
        const references = [] as BinaryReference[]; // references point to an index in the binary data where pointers are to be stored
    ...
    static async resolveBinaryReferences(writer: BinaryWriter, references: BinaryReference[], pointers: BinaryPointer[]) {
