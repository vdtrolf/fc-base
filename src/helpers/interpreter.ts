...
import {DIRECTION_UP, DIRECTION_DOWN, DIRECTION_LEFT, DIRECTION_RIGHT, DIRECTION_NONE,
    ...
    MOVES, ACTIVITY_NAMES, DIRECTION_NAMES } from "../constants";
...
export const getDirection = (command:string, activity:number) => {
    ...
    if (command.toUpperCase()[0] === "U") {
        ...
        let move = MOVES[DIRECTION_RIGHT]
        return {
            'activity': activity,
            'activityName': ACTIVITY_NAMES[activity],
...
export const findItem = (vpos:number,hpos:number,items: {}) => {
    // //
    for (let direction = 0; direction < 4; direction++) {
        let coord = (vpos + MOVES[direction][0]) * 100 + hpos + MOVES[direction][1]
        if (items[coord]) 
            return direction
    }
    return -1
...
export const interpretCommands = (commands: string[],vpos:number,hpos:number,cellsPos:{},fishesPos:{},gemsPos:{},garbagesPos:{}) => {
    ...
    if (commands.length === 1) {
        if (commands[0].toUpperCase()[0] == "E" ) {
            ...
            let foundDirection = findItem(vpos,hpos,fishesPos)
            if (foundDirection >= 0 ) {
                let move = MOVES[foundDirection]
                return {
            ...
            let direction = getDirection(commands[0], ACTIVITY_MOVING)
            ...
            } else if ( cellsPos[(vpos + direction.vmove) * 100 + hpos + direction.hmove] === 0 ) {
