...
export const cellBecomeOlder = ( cell: Cell, cells : Cell[], size : number, weather: number, evolutionSpeed: number) => {
    let smeltFactor = 6 - evolutionSpeed    

    if (cell.vpos == 0 || cells[cell.vpos - 1][cell.hpos].type > 0 )
        smeltFactor += 1
    if (cell.vpos == size - 1 || cells[cell.vpos + 1][cell.hpos].type > 0) 
        smeltFactor += 1
    if (cell.hpos == 0 || cells[cell.vpos][cell.hpos - 1].type > 0 )
        smeltFactor += 1
    if (cell.hpos == size - 1 || cells[cell.vpos][cell.hpos + 1].type > 0)
        smeltFactor += 1
